"""
api.py - Cric AI FastAPI Backend (Oracle Cloud Optimized)
"""
import json
import uuid
import shutil
...
# CORS Setup
ALLOWED_ORIGINS = os.getenv("ALLOWED_ORIGINS", "https://cricai-amber.vercel.app,http://localhost:3000").split(",")
app.add_middleware(
    CORSMiddleware,
...
async def get_result(aid: str):
    if aid in jobs_db:
        ...
        if j["status"] == "completed":
            with open(j["json_path"]) as f: r = json.load(f)
            r["api"] = {"analysisId": aid, "video": f"/video/{aid}", "annotatedVideo": f"/video/{aid}"}
            return r
    
    jp = RESULT_DIR / f"{aid}_phase1_result.json"
    if jp.exists():
        with open(jp) as f: return json.load(f)
